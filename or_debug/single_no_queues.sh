...
CLUSTER=$CLUSTER+"_no_queues"
REPO="/home/michaelbutler/go/src/github.com/cockroachdb/cockroach"
pwd
cd $REPO
pwd
#roachprod create $CLUSTER --nodes=1 --clouds aws --local-ssd=false --aws-ebs-volume-size=1000
roachprod put $CLUSTER:1 "artifacts/cockroach"
roachprod start $CLUSTER
roachprod sql $CLUSTER:1 -- -e "SET CLUSTER SETTING kv.queue.process.guaranteed_time_budget='1h'"
roachprod sql $CLUSTER:1 -- -e "SET CLUSTER SETTING jobs.debug.pausepoints = 'restore.before_do_download_files'"
roachprod sql $CLUSTER:1 -- -e "SET CLUSTER SETTING kv.consistency_queue.enabled = false"
roachprod sync
